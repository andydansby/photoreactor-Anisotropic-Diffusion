...
public:
	...
	{
		return "Anisotropic Diffusion algorithm by P. Perona, J. Malik is an edge preserving smoothing built on an iterative process.  The more iterations, the slower the filter is, beware of the higher iterations, as the filter will take a long time.";
	}
	...
	int GetUIParameters (UIParameters* pParameters)
	{
		...
		AddParameter( PARAM_ITERATIONS ,"Iterations", 15.0, 1.0, 100.0, TYPE_SLIDER, 0.0);//default  min   max
		AddParameter( PARAM_SIGMA ,"Sigma", 0.05, 0.0001, .1, TYPE_SLIDER, 0.0);//default  min   max
		AddParameter( PARAM_TYPE ,"Exponential|Quadratic", 0, 0, 1, TYPE_ONEOFMANY, 0);
	...
	{
		//List of Parameters
		int dIterations = GetValue(PARAM_ITERATIONS);// used to grab radius from control
		double nSigma = GetValue(PARAM_SIGMA);
		int type = GetValue(PARAM_TYPE);

		int iterations = (int)dIterations;
		float sigma = nSigma;
		...
		memset( temp, 0, sizeof(imagesize) );
		...
		for (int x = 0; x < nWidth; x++)
		{
			for (int y = 0; y < nHeight; y++)
			{
					...
					iterativeArray[nIdx + CHANNEL_R] = red / colorspace;
					iterativeArray[nIdx + CHANNEL_G] = green / colorspace;
					iterativeArray[nIdx + CHANNEL_B] = blue / colorspace;
			}//end x
		}//end y

		//This is our Anisotropic Diffustion routine
		for(int iter=0; iter<iterations; iter++) 
		{
			float redsigma = 0.0f;
			float greensigma = 0.0f;
			float bluesigma = 0.0f;
			...
			for(int y=0; y<nHeight; y++) 
			{
				for(int x=0; x<nWidth; x++) 
				{
					...
					temp[nIdx + CHANNEL_R] = 0.0;
					temp[nIdx + CHANNEL_G] = 0.0;
					temp[nIdx + CHANNEL_B] = 0.0;
					...
					for(int i=0; i<4; i++) 
					{
						...
						int nIdx2 = xx * 4 + yy * 4 * nWidth;

						if(xx < 0 || yy < 0 || xx >= nWidth || yy >= nHeight) continue;

						//acts like centerpixel
						float centerRed = iterativeArray[nIdx + CHANNEL_R];
						float centerGreen = iterativeArray[nIdx + CHANNEL_G];
						float centerBlue = iterativeArray[nIdx + CHANNEL_B];

						//acts like slider
						float sliderRed = iterativeArray[nIdx2 + CHANNEL_R];
						float sliderGreen = iterativeArray[nIdx2 + CHANNEL_G];
						float sliderBlue = iterativeArray[nIdx2 + CHANNEL_B];

						float reddifference = sliderRed - centerRed;
						float greendifference = sliderGreen - centerGreen;
						float bluedifference = sliderBlue - centerBlue;

						if (type == 0)//ok
						{
							redsigma = expf (- reddifference * reddifference / sigma2);
							greensigma = expf(- greendifference * greendifference / sigma2);
							bluesigma = expf (- bluedifference * bluedifference / sigma2);
						}
						if (type == 1)//ok
						{
							redsigma = 0.5f / (0.5f + ( reddifference * reddifference / sigma2));
							greensigma = 0.5f / (0.5f + ( greendifference * greendifference / sigma2));
							bluesigma = 0.5f / (0.5f + ( bluedifference * bluedifference / sigma2));
						}



						temp[nIdx + CHANNEL_R] += redsigma * (sliderRed - centerRed);
						temp[nIdx + CHANNEL_G] += greensigma * (sliderGreen - centerGreen);
						temp[nIdx + CHANNEL_B] += bluesigma * (sliderBlue - centerBlue);

						count++;
					}//end i

					temp[nIdx + CHANNEL_R] = iterativeArray [nIdx + CHANNEL_R] + lambda / (float) count * temp[nIdx + CHANNEL_R];
					temp[nIdx + CHANNEL_G] = iterativeArray [nIdx + CHANNEL_G] + lambda / (float) count * temp[nIdx + CHANNEL_G];
					temp[nIdx + CHANNEL_B] = iterativeArray [nIdx + CHANNEL_B] + lambda / (float) count * temp[nIdx + CHANNEL_B];

					//temp[nIdx + CHANNEL_R] = iterativeArray [nIdx + CHANNEL_R] - expf(-1 * (float) count * temp[nIdx + CHANNEL_R]);
					//temp[nIdx + CHANNEL_G] = iterativeArray [nIdx + CHANNEL_G] - expf(-1 * (float) count * temp[nIdx + CHANNEL_G]);
					//temp[nIdx + CHANNEL_B] = iterativeArray [nIdx + CHANNEL_B] - expf(-1 * (float) count * temp[nIdx + CHANNEL_B]);
				}//end x
			}//end y

			//copy iterativeArray for iterative process
			for (int x = 0; x< nWidth; x++)
			{
				for (int y = 0; y< nHeight; y++)
				{
					...
					iterativeArray[nIdx + CHANNEL_R] = temp [nIdx + CHANNEL_R];
					iterativeArray[nIdx + CHANNEL_G] = temp [nIdx + CHANNEL_G];
					iterativeArray[nIdx + CHANNEL_B] = temp [nIdx + CHANNEL_B];	
				}//end y
			}//end x
		}//end iterations
...
#pragma region
		for (int x = 0; x< nWidth; x++)
		{
			for (int y = 0; y< nHeight; y++)
			{
					...
					//float nR = dxdy [nIdx + CHANNEL_R] * colorspace;
					//float nG = dxdy [nIdx + CHANNEL_G] * colorspace;
					//float nB = dxdy [nIdx + CHANNEL_B] * colorspace;
...
{
	return "Andy's Anisotropic Diffusion";	
}
...
extern "C" __declspec(dllexport) char* GetPluginID()
{
	...
	return "com.lumafilters.Anisotropic";
